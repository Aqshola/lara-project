@extends('app')
@section('content')
    <h1 class="text-3xl font-bold">Update Patient</h1>
    <form class="flex flex-col gap-2 mt-10" method="POST" action="{{ route('patient.update', $data['patient_id']) }}">
        @csrf
        @method('PUT')
        @if ($errors->any())
        ...
        <div class="flex flex-col w-56">
            ...
            <input required type="text" id="idpatient" name="idpatient" readonly
                class="border border-black rounded px-2 read-only:bg-gray-200" value="{{ $data['patient_id'] }}">
            @error('idpatient')
        ...
        <div class="flex flex-col w-56">
            ...
            <input onkeypress="return onlyNumbers(event)" type="text" id="phonepatient" name="phonepatient"
                class="border border-black rounded px-2" required value="{{ $data['phone'] }}">
            @error('phonepatient')
                <p class="text-red-400 font-bold">{{ $message }}</p>
            @enderror
        </div>
